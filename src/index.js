...
function createCountriesItemsMarcup(countries) {
  if (Number(countries.length) > 10) {
    ...
    const markup = countries
      .map(({ name, flags }) => {
        return `<li class="country-item"><img class="country-img" src="${flags.svg}" alt="flag of ${name.official}" width="50" height="100%"><p class="country-title">${name.official}</p></li>`;
      })
      .join('');
